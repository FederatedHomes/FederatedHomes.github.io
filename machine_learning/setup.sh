...
require_client_ca_certificate() {
  local ca_file="${TLS_CERTIFICATE_HOST_DIR:-./certificates/tls}/ca.crt"
  [ -f "$ca_file" ] || {
...
create_starter_tls_material() {
  [ "$1" = server ] || return 0
  local tls_dir="${TLS_CERTIFICATE_HOST_DIR:-./certificates/tls}" starter_host="${SUPERLINK_HOST:-}"
  [ -n "$starter_host" ] || {
...
create_starter_client_auth() {
  local client_id="$1" auth_dir="${SUPERNODE_AUTH_HOST_DIR:-./certificates/auth}"
...
register_configured_clients() {
  ...
  local auth_dir="${SUPERNODE_AUTH_HOST_DIR:-./certificates/auth}"
  [ -f "$compose_file" ] || {
...
from src.deployment_config import load_deployment_config
try:
    c = load_deployment_config(role="${DEPLOYMENT_ROLE:-server}")
    print(f"Fleet API: {c.superlink_address}")
    print(f"Control API: {c.superlink_control_address}")
    print(f"TLS host directory: {c.tls_certificate_host_dir}")
    print(f"SuperNode auth host directory: {c.supernode_auth_host_dir}")
    print(f"SuperLink state host directory: {c.superlink_state_host_dir}")
except Exception as exc:
    print(f"Configuration validation: {exc}")
PY
}
...
FederatedHomes deployment setup
1) Prepare host
2) Start server infrastructure
3) Start client infrastructure
4) Run tests
