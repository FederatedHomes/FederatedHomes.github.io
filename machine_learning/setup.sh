...
prepare_host() {
  load_environment; read_clients; require_production_profile
...
generate_server_compose() {
  load_environment; read_clients; require_production_profile; ensure_host_dependencies server; export DEPLOYMENT_ROLE=server
  [ -n "${SUPERLINK_HOST:-}" ] || { echo "ERROR: Set SUPERLINK_HOST in .env." >&2; return 1; }
  prepare_flower_config
  python3 scripts/generate_compose.py --config clients.yml --output docker-compose.server.yml --profile production --role server
}

generate_client_compose() {
  load_environment; read_clients; require_production_profile; ensure_host_dependencies client; export DEPLOYMENT_ROLE=client
  local client_id="${CLIENT_ID:-}"; [ -n "$client_id" ] || client_id="$(select_client_id)"; export CLIENT_ID="$client_id"
  [ -n "${SUPERLINK_HOST:-}" ] || { echo "ERROR: Set SUPERLINK_HOST in .env." >&2; return 1; }
  prepare_flower_config
  python3 scripts/generate_compose.py --config clients.yml --output docker-compose.client.yml --profile production --role client --client-id "$client_id"
}
...
except Exception as exc: print(f"Configuration validation failed: {exc}")
PY
  cat clients.yml
}
...
main_menu() {
  while true; do
    echo; echo "FederatedHomes Flower deployment setup"; echo "  1) Prepare host"; echo "  2) Generate server Compose"; echo "  3) Generate client Compose"; echo "  4) Start server infrastructure"; echo "  5) Start client infrastructure"; echo "  6) Run tests"; echo "  7) Show configuration"; echo "  8) Exit"
